...
function validateSaveDailyOrderPayload(intent) {
    ...
    var errors = [];
    if (Array.isArray(payload.items) && payload.items.some(function(item) { return item && (item.productMatchPending === true || item.productMatchUnavailable === true); })) {
        errors.push('Match or replace unavailable historical products before saving this order.');
    }
